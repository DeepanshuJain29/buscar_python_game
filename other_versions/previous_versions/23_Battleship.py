...
def GameWonAnimation(): # screen to draw Game Won Animation.
    ...
    GameWonText = FontObj_Over.render('YOU WIN', True, WHITE)
...
class Gamer :
    ...
    def __init__(self, name):
        self.Name = name
        self.Score = Gamer.scrDict
    def AddScore(self, scr):
        DateTime = str(now.tm_mday)+'-'+str(now.tm_mon)+'-'+str(now.tm_year)+'::'+str(now.tm_hour)+':'+str(now.tm_min)+':'+str(now.tm_sec)
        Gamer.scrDict[DateTime] = scr
...
while Choice <> Ch[2]:
    if Choice == Ch[0]:
            ...
            while START_BUTTON_ON: # Finally Start Button is clicked (or, it is 'ON').
                ...
                if mouseClicked:
                    ...
                    if ReturnToHome_Rect.collidepoint(mousex, mousey):
                        ...
                        #...clicked on any box...
                        SCORE -= 5
                        if chance == 7: #...and if chance is equal to 7 (Finally..)...
        ...
        NAME = raw_input('here: ')
        if NAME in RecData:
            print ReturnItsData(NAME)
        else:
            print 'Sorry! No record for ',NAME,'...'
