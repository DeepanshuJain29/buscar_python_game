...
def DrawInstrucButton(x = False): # Do not draw button unless x is given true.
    ...
    if not x: # when x is false give StartButton's rect.
        ...
        for size in range(25): # First animation. (Of resizing start button's background)
            ...
            #--------------------------------------------------------------------------------
            #pygame.time.wait(10)
            width = 150 + size # Width of surface increases after every loop...
            height = 35 + size #...and same for height
            BG = pygame.Surface((width, height))
            BG.fill((180,180,255))
            left = left - (width/2) #------ Here TOP - LEFT are adjusting with increase in size -
            top = top - (height/2)
            ...
            START_COLOR = (0,0,255)
            pygame.draw.rect(DISPLAYSURF, WHITE, InstrucButton)
        ...
        for size in range(25): # First animation. (Of resizing start button's background)
            ...
            #--------------------------------------------------------------------------------
            #pygame.time.wait(10)
            width = 160 + size # Width of surface increases after every loop...
            height = 35 + size #...and same for height
            BG = pygame.Surface((width, height))
            BG.fill(WHITE)
            left = left - (width/2) #------ Here TOP - LEFT are adjusting with increase in size -
...
def IntroAnimation(x = False): # Animation played when game is started.
    ...
    for pos in range(SCR_HEIGHT/2): # Animation to change position of text...
        ...
        pygame.draw.rect(DISPLAYSURF, WHITE, (SCR_WIDTH/2 - 50,0, 100, SCR_HEIGHT/2))
...
while True: # Main Game Loop ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    while not START_BUTTON_ON: # Loop unless Start Button is clicked.
        ...
        InstrucButtonRect = DrawInstrucButton()
        StartButton_Rect = DrawStartButton()
        if InstrucButtonRect.collidepoint(mousex, mousey):
