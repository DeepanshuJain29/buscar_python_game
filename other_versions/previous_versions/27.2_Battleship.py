...
def GameWonAnimation(x): # screen to draw Game Won Animation.
    # Transparent Overlay---------------------------------------------------------
    ...
    DISPLAYSURF.blit(Overlay, (0,0))    # (0,0) are the top-left coordinates
    pygame.display.update()
    Level_UP_SIZE = 90 # Font size for 'LEVEL:X' text
    FontObj_Level = pygame.font.Font('PoplarStd.otf', Level_UP_SIZE)
    LevelUpText = FontObj_Over.render('LEVEL: '+str(x+1), True, WHITE)
    LevelUpText_Rect = LevelUpText.get_rect()
    LevelUpText_Rect.center = (SCR_WIDTH/2, SCR_HEIGHT/2)
    DISPLAYSURF.blit(LevelUpText, LevelUpText_Rect)
    pygame.display.update()
...
def FoundText(found, total):
    ...
    for alpha in range(25):
        ...
        pygame.display.update()
    #-----------------------------------------------------------------------------
    FONT_SIZE = 60 # Font size for 'GameOver' text
...
def GameWon(board): # check if Game(is)Over. (it goes through each box in OUR_BOXES...
...
while Choice <> Ch[2]:
    ...
    if Choice == Ch[0]:
        os.system('cls')
        NAME = raw_input("So, What should I name you? (or type 'B' or 'b' to go back to menu) ")
        NAMES = []
        for data in Rec:
            NAMES.append(data[0])
        while NAME in NAMES or len(NAME) == 0:
            os.system('cls')
            if NAME in NAMES:
                    ...
                    if mouseClicked:
                        ...
                        if ReturnToHome_Rect.collidepoint(mousex, mousey):
                            ...
                            if not GameWon(board):
                                ...
                                GameWonAnimation(level)#...Display GameWonAnimation.
                                level+=1
                                total = level+2
                                RR = randint(0,100)
                                RG = randint(0,100)
                            ...
                            #...clicked on any box...
                            SCORE -= 5
                            if chance == 10: #...and if chance is equal to 10 (Finally..)...
