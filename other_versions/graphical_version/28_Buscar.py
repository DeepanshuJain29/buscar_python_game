...
try:
    ...
    BOXSIZE = 65 # length and breadth of box
    GAPSIZE = 20 # gap b/w boxes
    ...
    BGS = pygame.image.load('BGStart.jpg')
    GREEN = (0,255,0)
    BLUE = (0,0,255)
    ...
    def RandomRock():
        ...
        if Num == 1:
            ...
            rockD = pygame.image.load('1dark.png')
        elif Num == 2:
            rock = pygame.image.load('2.png')
            rockD = pygame.image.load('2dark.png')
        elif Num == 3:
            rock = pygame.image.load('3.png')
            rockD = pygame.image.load('3dark.png')
        else:
            rock = pygame.image.load('4.png')
            rockD = pygame.image.load('4dark.png')
        return (rock, rockD)

    def RandomRocks(BOARD_WIDTH, BOARD_HEIGHT):
        del Rocks[::]
    ...
    def DrawBoard(board,Rocks, SEL_BOX_LIST, x = 'default'): # drawing board
                ...
                for box in row: # on box
                    ...
                    if ThisBox in SEL_BOX_LIST: # first se that if ThisBox is in SEL_BOX_LIST.
                        if ThisBox in OUR_BOXES_RECT: # next check if it is also one of the...
                            ...
                            DISPLAYSURF.blit(gold, ThisBox)
                            n+=1
                        else:
                            rock = Rocks[n][1]
                            DISPLAYSURF.blit(rock, ThisBox)
                            n+=1
    ...
    def ReturnToHome(x = False): # Draw ReturnToHome button.
        ...
        if not x: # just return its rect if x is False..
            ...
            pygame.draw.polygon(DISPLAYSURF, BLACK, Coordinates)
            FontObj = pygame.font.Font('PoplarStd.otf', FONT_SIZE)
            RTHText = FontObj.render('RETURN TO MENU', True, BLACK)
    ...
    def DrawStartButton(x = False): # Do not draw button unless x is given true.
        ...
        if not x: # when x is false give StartButton's rect.
            ...
            pygame.draw.rect(DISPLAYSURF, SWAPCOLOR, StartButton)
            FontObj_Start = pygame.font.Font('PoplarStd.otf', START_FONT_SIZE)
            StartButtonText = FontObj_Start.render('START', True, WHITE, SWAPCOLOR)
    ...
    def DrawInstrucButton(x = False): # Do not draw button unless x is given true.
        ...
        if not x: # when x is false give InstrucButton's rect.
            ...
            pygame.draw.rect(DISPLAYSURF, SWAPCOLORins, InstrucButton)
            FontObj_Instruc = pygame.font.Font('PoplarStd.otf', INSTRUC_FONT_SIZE)
            InstrucButtonText = FontObj_Instruc.render('INSTRUCTIONS', True, WHITE, SWAPCOLORins)
            ...
            pygame.display.update()
            pygame.time.wait(200)
            if SWAPCOLORins == (0,0,0):
                SWAPCOLORins = (20,20,20)
            else:
                SWAPCOLORins = (0,0,0)
            #--------------------------------------------------------------
    ...
    def Enter_Text(): # write the text - "Press 'ENTER' or Click on 'START' to Start the game".
        ...
        COLOR = (5, 5, 5) 
        EnterText = FontObj_1.render("Press 'ENTER' or Click on 'START' to Start the game", True, COLOR)
    ...
    def IntroPage(): # Page shown when game is started.
        INTRO_FONT_SIZE = 150 # Font size for 'Buscar' text
        FontObj = pygame.font.Font('gomarice_kaiju_monster.ttf', INTRO_FONT_SIZE)        
        Buscar = FontObj.render("Buscar!", True, BGCOLOR)
    ...
    def DrawBlinkBox(x, y, board):
        for row in board:
            for box in row:
                left,top = LeftTopOf(board, box)
                AnyBox = pygame.Rect(left, top, BOXSIZE, BOXSIZE)
                if AnyBox.collidepoint(x, y): # if we come up with our box..
    ...
    def PrintChance(x): # Prints the chance.
        ...
        ChanceText = FontObj_Chance.render('CHANCE :'+str(x), True, BLACK)
    ...
    RandomRocks(BOARD_WIDTH, BOARD_HEIGHT)
            ...
            if NAME in ['B','b']: # if user enters 'b' or 'B' as NAME then take him to main menu.
                ...
                while START:
                    ...
                    if not START_BUTTON_ON: # Show IntroPage if START_BUTTON is not 'ON'
                        DISPLAYSURF.blit(BGS, (0,0))# Coloring the window background 
                    ...
                    if not START: # check if START is 'False'...
                        ...
                        DrawBoard(board,Rocks, SEL_BOX_LIST) # Draw Board.
                    ...
                    while START_BUTTON_ON: # Finally Start Button is clicked (or, it is 'ON').
                        ...
                        PrintChance(chance)
                        board = CreateBoard(BOARD_HEIGHT, BOARD_WIDTH) # Here is our board.
                        DrawBoard(board, Rocks, SEL_BOX_LIST)
                        #--------------------------------------------------------------------------------------
                        ...
                        if mouseClicked:
                            ...
                            if ReturnToHome_Rect.collidepoint(mousex, mousey): # if user clicks on ReturnToHome button.
                                ...
                                #...clicked on our box...
                                SCORE += 10
                                if not GameWon(board): # if user have not completed the level yet...
                                    found += 1
                                    #FoundText(found, total) #...just show FoundText...
